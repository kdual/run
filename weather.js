...
export async function searchLocations(query, signal) {
  ...
  const results=[...new Map(batches.flat().map(r=>[r.id,r])).values()];
  return results.sort((a,b)=>{
    const rank=r=>(r.country_code==='KR'?100:0)+(r.name===query?35:0)+(r.name===`${query}구`?50:0)+(r.name===`${query}시`?45:0)+(r.feature_code?.startsWith('ADM')?40:0)+(r.admin1?.includes('서울')?25:0)+Math.min(20,Math.log10((r.population||1)+1)*3);
