...
function render(){
  ...
  renderHero(current,best,todayScore,futureBest);renderCurrent(current,weather);renderBest(best);renderHourly();renderComparison(todayScore,tomorrowScore);renderCoach(current,futureBest);renderWeekly(weather,hourly);renderWorkouts(current);renderGear(current);
...
function renderHourlyDetail(r){const overall=scoreCondition(r.score);const date=new Intl.DateTimeFormat('ko-KR',{month:'long',day:'numeric',weekday:'short'}).format(new Date(`${r.time}:00+09:00`));const forecastAir=r.air_quality_estimated;const measuredIndex=forecastAir?null:r.air_quality_index;const metrics=[['temperature_2m','기온',formatValue(r.temperature_2m,'°C',1),r.temperature_2m],['apparent_temperature','체감',formatValue(r.apparent_temperature,'°C',1),r.apparent_temperature],['relative_humidity_2m','습도',formatValue(r.relative_humidity_2m,'%'),r.relative_humidity_2m],['dew_point_2m','이슬점',formatValue(r.dew_point_2m,'°C',1),r.dew_point_2m],['precipitation_probability','강수확률',formatValue(r.precipitation_probability,'%'),r.precipitation_probability],['wind_speed_10m','바람',formatValue(r.wind_speed_10m,' km/h',1),r.wind_speed_10m],['uv_index','UV',formatValue(r.uv_index,'',1),r.uv_index],['air_stagnation_index','대기정체',formatValue(r.air_stagnation_index,'',0),r.air_stagnation_index],['pm2_5','PM2.5 실측',formatValue(r.pm2_5,' μg/m³',1),r.pm2_5],['air_quality_index','통합대기지수',forecastAir?'미래 실측값 없음':formatValue(measuredIndex,''),measuredIndex]];const forecastNote=forecastAir?`<p class="hourly-air-note">권역 PM 일일 예보: <strong>${escapeHtml(r.air_forecast_grade||'정보 없음')}</strong> · 시간별 통합대기지수 예보는 아닙니다.</p>`:'';$('hourlyDetail').innerHTML=`<div class="hourly-detail-heading"><strong>${date} ${timeOnly(r.time)} 상세 지표</strong><span class="hourly-score condition-${overall.key}">${r.score}점 · ${overall.label}</span></div><div class="condition-legend"><span class="condition-good">좋음·낮음</span><span class="condition-normal">보통</span><span class="condition-bad">안좋음·높음</span><span class="condition-very-bad">아주 안좋음·매우 높음</span></div><div class="hourly-metrics">${metrics.map(([key,label,value,raw])=>{const c=conditionFor(key,raw);return`<span class="condition-${c.key}"><small>${label}</small><strong>${value}</strong><em>${forecastAir&&key==='air_quality_index'?'시간별 값 없음':c.label}</em></span>`;}).join('')}</div>${forecastNote}`;}

function renderComparison(today,tomorrow){
...
function calculatorTemplate(){const html=state.calcMode==='pace'?`<div class="field"><label for="paceInput">페이스 (분:초/km)</label><input id="paceInput" type="text" value="500" inputmode="numeric" autocomplete="off" aria-describedby="paceInputHint"><small id="paceInputHint">숫자만 입력: 530 → 5:30, 600 → 6:00 · 5:30도 가능</small></div>`:`<div class="field"><label for="speedInput">속도 (km/h)</label><input id="speedInput" type="text" inputmode="decimal" autocomplete="off" value="12" aria-describedby="speedInputHint"><small id="speedInputHint">소수점 입력 가능: 10.5 km/h</small></div>`;$('calculatorInputs').innerHTML=html;$('calculatorInputs').querySelectorAll('input').forEach(el=>el.addEventListener('input',calculate));calculate();}
function calculate(){const pace=state.calcMode==='pace'?parsePaceInput($('paceInput')?.value):null;const speed=state.calcMode==='speed'?Number($('speedInput')?.value.trim().replace(',','.')):null;const data=state.calcMode==='pace'?(pace===null?null:fromPace(pace)):(Number.isFinite(speed)?fromSpeed(speed):null);if(!data||data.paceSeconds<120||data.paceSeconds>1800){$('calculatorError').textContent=state.calcMode==='pace'?'페이스를 200–3000 또는 2:00–30:00 형태로 입력해 주세요.':'속도를 2–30 km/h 범위에서 입력해 주세요.';$('resultPace').textContent='--:--/km';$('resultSpeed').textContent='-- km/h';$('splitResults').innerHTML='';return;}$('calculatorError').textContent='';$('resultPace').textContent=renderPace(data.paceSeconds);$('resultSpeed').textContent=`${round(data.speed,1)} km/h`;$('splitResults').innerHTML=Object.entries(data.splits).map(([n,s])=>`<div class="split-item"><span>${n}</span><strong>${secondsToClock(s)}</strong></div>`).join('');}
