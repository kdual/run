...
export function renderHourlyChart(container, rows, onSelect) {
  ...
  const dismiss=()=>{tooltip.hidden=true;guide.hidden=true;container.querySelectorAll('.point').forEach(p=>p.classList.remove('selected'));};
  ...
  container.querySelectorAll('.point-hit').forEach(point=>{
    point.addEventListener('click',event=>{event.stopPropagation();activate(point);});
  ...
  const dismissOnBlank=event=>{if(!event.target.closest('.point-hit'))dismiss();};
  document.addEventListener('click',dismissOnBlank);
  container._chartCleanup=()=>document.removeEventListener('click',dismissOnBlank);
